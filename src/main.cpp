...
namespace
{
    ...
    void selected_option(const uint8_t SelectedOption, const uint8_t NumberOptions,
                         ...
                         bn::sprite_text_generator TextGenerator, const bn::array<bn::string<32>, 2> Texts, bn::fixed &angle,
                         const bn::fixed angle_inc)
    {
        ...
        for (uint8_t option = 0; option < NumberOptions; ++option)
        {
            if (option != SelectedOption)
            {
                ...
                TextGenerator.generate(0, 15 * option, Texts[option], TextSprites);
            }
            else
            {
                ...
                TextGenerator.generate(0, 15 * option, Texts[option], SelectedTextSprites);
    ...
    void selected_option(const uint8_t SelectedOption, const uint8_t NumberOptions,
                         bn::vector<bn::sprite_ptr, 32> &TextSprites, bn::vector<bn::sprite_ptr, 32> &SelectedTextSprites,
                         bn::sprite_text_generator TextGenerator, const bn::array<bn::string<32>, 9> Texts, bn::fixed &angle,
                         const bn::fixed angle_inc)
    {
        ...
        for (uint8_t option = 0; option < NumberOptions; ++option)
        {
            ...
            {
                TextGenerator.set_palette_item(bn::sprite_items::common_variable_8x16_font.palette_item());
                TextGenerator.generate(-40, -50 + (15 * option), Texts[option], TextSprites);
            }
            else
            {
                TextGenerator.set_palette_item(bn::sprite_items::variable_8x16_font_red.palette_item());
                TextGenerator.set_one_sprite_per_character(true);
                TextGenerator.generate(-20, -50 + (15 * option), Texts[option], SelectedTextSprites);
                TextGenerator.set_one_sprite_per_character(false);
            }
        }
    ...
    void album_selection(bn::vector<bn::sprite_ptr, 32> &TextSprites, bn::vector<bn::sprite_ptr, 32> &SelectedTextSprites,
                         bn::sprite_text_generator TextGenerator, bn::fixed &angle,
                         const bn::fixed angle_inc)
    {
    ...
    while (true)
    {
        ...
        if (bn::keypad::a_pressed())
        {
            ...
            if (SelectedOption == 0)
            {
                TextSprites.clear();
                SelectedTextSprites.clear();
                album_selection(TextSprites, SelectedTextSprites, TextGenerator, angle, angle_inc);
            }
        }
